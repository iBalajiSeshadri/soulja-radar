...
def clean_snippet_text(text, max_len=110):
...
def extract_headline_subject(raw_text):
    """
    Surgically extracts the exact primary subject player from RotoWire/CBS headlines.
    Example: "Patriots' Behren Morton: Solid in extended action" -> "Behren Morton"
    Example: "Bills' Ray Davis: Scores in second preseason game" -> "Ray Davis"
    """
    if ':' not in raw_text:
    ...
    # Strip team possessive prefixes (e.g. "Patriots'", "Bills'", "49ers'", "Buccaneers'")
...
p_db = {}
try:
    p_db = requests.get("https://api.sleeper.app/v1/players/nfl", timeout=10).json()
    ...
    for pid, pdata in p_db.items():
        ...
        if status in ['IR', 'PUP', 'Sus', 'Out'] or inj_status in ['IR', 'Out', 'PUP', 'Doubtful']:
            ...
            camp_overrides[p_name] = {
                "multiplier": 0.88,
                "type": "QUESTIONABLE",
                "note": f"🩹 OFFICIAL INJURY WIRE: Questionable ({inj_body})"
            }
            sleeper_injuries_found += 1
            
    print(f"✓ Ingested {sleeper_injuries_found} official injury designations!")

# 3. Source B: CBS Sports Stream (Surgical Subject Extraction)
...
# 4. Source C: Sleeper 24-Hour Live Trending Waiver Adds (Exact ID Match)
...
print(f"\n✅ SUCCESS: Saved {len(camp_overrides)} subject-isolated overrides to 'camp_overrides.json'!")